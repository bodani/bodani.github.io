...
加密字段最大的痛点就是**无法直接检索**。因为 `pgp_sym_encrypt()` 每次加密都会使用随机 IV，同一个明文每次加密出来的密文都不一样，所以：

- ❌ `WHERE phone_encrypted = pgp_sym_encrypt('13800001234', 'key')` 永远查不到结果
- ❌ `WHERE phone_encrypted LIKE '138%'` 无法做前缀匹配
- ❌ `ORDER BY phone_encrypted` 排序无意义
...
> 💡 **结论**：生产环境**务必使用 HMAC**，密钥由应用层管理，不要写入数据库。
...
加密后的字段**无法做范围查询**（`>`、`<`、`BETWEEN`）和**排序**（`ORDER BY`），因为密文与明文的大小关系不一致。
...
密钥规范
pgcrypto用AES256算法的时候，标准密钥是32字节长度的密码学安全随机串，你不用自己想，一行命令就能生成：
```
openssl rand -base64 32
```
✅ 这个生成的串是完全随机的，没有任何规律，暴力破解需要的时间比宇宙年龄还长，直接用就好。
❌ 绝对不要用生日、域名、固定字符串当密钥，也不要自己改随机串的内容。
