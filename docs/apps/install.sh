...
if [ "$DOCKER_POOL_FORCE_OVERRIDE" = true ] || [ "$EXISTING_POOL_CONFIGURED" = false ]; then
    ...
    if [ -f /etc/docker/daemon.json ]; then
        ...
        if [ "$CURRENT_POOL_BASE" = "$DOCKER_ADDRESS_POOL_BASE" ] && [ "$CURRENT_POOL_SIZE" = "$DOCKER_ADDRESS_POOL_SIZE" ]; then
            ...
            # Preserve existing user configurations (registry-mirrors, exec-opts, etc.)
            EXISTING_CONFIG=$(cat /etc/docker/daemon.json 2>/dev/null || echo '{}')
            NEW_CONFIG=$(echo "$EXISTING_CONFIG" | jq --arg base "$DOCKER_ADDRESS_POOL_BASE" --argjson size "$DOCKER_ADDRESS_POOL_SIZE" '
            {
...
EOL
    ...
    if [ -f /etc/docker/daemon.json ] && jq -e '.["log-driver"] == "json-file" and .["log-opts"]["max-size"] == "10m" and .["log-opts"]["max-file"] == "3"' /etc/docker/daemon.json >/dev/null 2>&1; then
        ...
        # Use jq to merge with existing configuration
        echo " - Updating log configuration while preserving existing settings..."
        EXISTING_CONFIG=$(cat /etc/docker/daemon.json 2>/dev/null || echo '{}')
